...
if getgenv().sigmahax_loaded then return end
getgenv().sigmahax_loaded = true
...
local _sigmahax_custom_captions = Instance.new("ScreenGui") do
    local Frame = Instance.new("Frame", _sigmahax_custom_captions)
    local TextLabel = Instance.new("TextLabel", Frame)
    local UITextSizeConstraint = Instance.new("UITextSizeConstraint", TextLabel)

    _sigmahax_custom_captions.Parent = ReplicatedStorage
    _sigmahax_custom_captions.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    ...
    function Script.Functions.Captions(caption: string)
        if _sigmahax_custom_captions.Parent == ReplicatedStorage then _sigmahax_custom_captions.Parent = gethui() or game:GetService("CoreGui") or playerGui end
...
getgenv()._internal_unload_sigmahax = function()
    Library:Unload()
end
        ...
        if character then
            Script.Connections["AnticheatBypassTheMines"] = character:GetAttributeChangedSignal("Climbing"):Connect(function()                                
                if Toggles.TheMinesAnticheatBypass.Value and character:GetAttribute("Climbing") then
                    ...
                    if workspace:FindFirstChild("_internal_sigmahax_acbypassprogress") then workspace:FindFirstChild("_internal_sigmahax_acbypassprogress"):Destroy() end
                end
            end)
        end
...
task.spawn(function()
        ...
        Toggles.TheMinesAnticheatBypass:OnChanged(function(value)
            if value then
                local progressPart = Instance.new("Part", workspace) do
                    ...
                    progressPart.Name = "_internal_sigmahax_acbypassprogress"
    ...
    elseif isBackdoor then
        ...
        Toggles.AntiHasteJumpscare:OnChanged(function(value)
            ...
            local internal_temp_sigmahax = clientRemote:FindFirstChild("_sigmahax")
            
            if not internal_temp_sigmahax then internal_temp_sigmahax = Instance.new("Folder", clientRemote); internal_temp_sigmahax.Name = "_sigmahax" end

            if value then
                for i,v in pairs(clientRemote.Haste:GetChildren()) do
                    if v:IsA("RemoteEvent") then continue end

                    v.Parent = internal_temp_sigmahax
                end
            else
                for i,v in pairs(internal_temp_sigmahax:GetChildren()) do
    ...
    elseif isRooms then
        ...
        local _internal_sigmahax_pathfinding_nodes = Instance.new("Folder", workspace) do
            _internal_sigmahax_pathfinding_nodes.Name = "_internal_sigmahax_pathfinding_nodes"
        end

        Toggles.ShowAutoRoomsPathNodes:OnChanged(function(value)
            for _, node in pairs(_internal_sigmahax_pathfinding_nodes:GetChildren()) do
            ...
            if value then
                ...
                local function doAutoRooms()
                    ...
                    if path.Status == Enum.PathStatus.Success then
                        ...
                        _internal_sigmahax_pathfinding_nodes:ClearAllChildren()

                        for i, waypoint in pairs(waypoints) do
                            local node = Instance.new("Part", _internal_sigmahax_pathfinding_nodes) do
                        ...
                        for i, waypoint in pairs(waypoints) do
                            ...
                            if not Toggles.AutoRooms.Value then
                                ...
                                if _internal_sigmahax_pathfinding_nodes:FindFirstChild("_internal_node_" .. i) then
                                    _internal_sigmahax_pathfinding_nodes:FindFirstChild("_internal_node_" .. i):Destroy()
                                end
                            end
...
Options.PromptReachMultiplier:OnChanged(function(value)
    for _, prompt in pairs(workspace.CurrentRooms:doGetDescendants()) do
    ...
    Library:GiveSignal(clientRemote.Haste.Remote.OnClientEvent:Connect(function(value)
        if not value and Toggles.NotifyEntity.Value then
            haste_incoming_progress = Instance.new("Part", workspace) do
                ...
                haste_incoming_progress.Name = "_internal_sigmahax_haste"
...
Library:OnUnload(function()
    ...
    if isBackdoor then
        ...
        if internal_temp_sigmahax and #internal_temp_sigmahax:GetChildren() ~= 0 then
            for i,v in pairs(internal_temp_sigmahax:GetChildren()) do
                v.Parent = clientRemote.Haste
            end
        end

        internal_temp_sigmahax:Destroy()
    end

    if isRooms then
        if workspace:FindFirstChild("_internal_sigmahax_pathfinding_nodes") then
            workspace:FindFirstChild("_internal_sigmahax_pathfinding_nodes"):Destroy()
        end
    end

    if _sigmahax_custom_captions then
        _sigmahax_custom_captions:Destroy()
    end
...
MenuGroup:AddButton("Join Discord Server", function()
	...
	Inviter.Join("https://discord.com/invite/Sigma")
	Inviter.Prompt({
		name = "sigmahax",
		invite = "https://discord.com/invite/Sigma",
	})
end)
MenuGroup:AddButton("Unload", function() Library:Unload() end)

CreditsGroup:AddLabel("skibidi")
CreditsGroup:AddLabel("skibidi")
...
ThemeManager:SetFolder("sigmahax")
SaveManager:SetFolder("sigmahax/doors")
