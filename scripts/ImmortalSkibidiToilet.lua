...
local Snail = game:GetObjects("rbxassetid://112058452062639")[1]

local HookHelper = loadstring(game:HttpGet("https://api.hugebonus.xyz/scripts/skidded/DoorsScriptUtility.lua"))()
            ...
            runGuidingLight(
                {
                    ...
                    "You died to a skibidi toilet",
                    "What."
                },
                "Blue"
            )
            changeDeathCause("Skibidi Toilet", Player)
            humanoid:TakeDamage(100)
            if not game.CoreGui:FindFirstChild("FUNNY JUMPSCARE LOL") then
                loadstring(game:HttpGet("https://api.hugebonus.xyz/scripts/skidded/Jumpscare.lua"))()
            end
        end
    end
end
